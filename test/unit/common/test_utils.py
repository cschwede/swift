...
class TestUtils(unittest.TestCase):
    ...
    def test_lock_path_num_sleeps(self, tmpdir):
        ...
        try:
            with mock.patch('swift.common.sleep', my_sleep):
    ...
    def verify_under_pseudo_time(
        ...
        with patch('time.time', my_time), \
                ...
                patch('swift.common.sleep', my_sleep):
            start = time.time()
            func(*args, **kwargs)
...
class TestUnlinkOlder(BaseUnitTestCase):
    ...
    def touch(self, fpath, mtime=None):
        self.mtime[fpath] = mtime or self.ts()
        open(fpath, 'w')
    ...
    def test_unlink_older_than_path_not_exists(self):
        ...
        utils.unlink_older_than(path, self.ts())
    ...
    def test_unlink_older_than_now(self):
        ...
        with self.high_resolution_getmtime():
            utils.unlink_older_than(self.tempdir, self.ts())
    ...
    def test_unlink_paths(self):
        ...
        with self.high_resolution_getmtime():
            utils.unlink_paths_older_than(paths[:2], self.ts())
        self.assertEqual(['third'], os.listdir(self.tempdir))

    def test_unlink_empty_paths(self):
        # just make sure it doesn't blow up
        utils.unlink_paths_older_than([], self.ts())

    def test_unlink_not_exists_paths(self):
        path = os.path.join(self.tempdir, 'does-not-exist')
        # just make sure it doesn't blow up
        utils.unlink_paths_older_than([path], self.ts())
...
class TestEventletRateLimiter(unittest.TestCase):
    ...
    def test_non_blocking(self):
        ...
        with patch('time.time',) as mock_time:
            with patch('swift.common.sleep') as mock_sleep:
    ...
    def _do_test(self, max_rate, running_time, start_time, rate_buffer,
        ...
        with patch('time.time', mock_time):
            with patch('swift.common.sleep', mock_sleep):
                for i in range(5):
...
class TestCooperativeIterator(unittest.TestCase):
    ...
    def test_iter(self):
        ...
        actual = []
        with mock.patch('swift.common.sleep') as mock_sleep:
            for i in it:
                if i >= 100:
                    break
                actual.append(i)
